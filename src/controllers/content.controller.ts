...
export const reorderContents = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const orderIds = order.map((o: { contentId: string }) => o.contentId);
    ...
    await prisma.$transaction([
      ...
      ...order.map(
        (item: { contentId: string; position: number }, index: number) =>
          prisma.projectContent.update({
            where: { id: item.contentId },
            data: { position: -(index + 1) },
          }),
      ),
      // Then set to actual positions
      ...order.map((item: { contentId: string; position: number }) =>
        prisma.projectContent.update({
          where: { id: item.contentId },
