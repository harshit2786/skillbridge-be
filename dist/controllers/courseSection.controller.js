...
export const reorderCourseSections = async (req, res) => {
    try {
        const { courseId } = req.params;
        ...
        await prisma.$transaction([
            ...order.map((item, index) => prisma.courseSection.update({
                where: { id: item.sectionId },
                data: { order: -(index + 1) },
            })),
            ...order.map((item) => prisma.courseSection.update({
                where: { id: item.sectionId },
...
//# sourceMappingURL=courseSection.controller.js.map