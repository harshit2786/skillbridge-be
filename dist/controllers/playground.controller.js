...
export const sendMessage = async (req, res) => {
    ...
    try {
        ...
        const sourcesData = await Promise.all(Array.from(sourceMap.values()).map(async (source) => {
        ...
        const context = searchResults
            .map((r, i) => `[Source ${i + 1}: ${r.payload.filename}]\n${r.payload.chunk}`)
            .join("\n\n---\n\n");
        ...
        if (chat.messages.length === 0) {
            const titleStream = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Generate a short title (max 6 words) for this chat based on the user's message. Return only the title, nothing else.",
                    },
