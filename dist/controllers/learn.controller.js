...
function validateCourseAnswer(type, data, answer) {
    switch (type) {
        case "mcq": {
        ...
        case "fill_ups": {
            ...
            const expectedBlanks = data.blanks || [];
            ...
            for (const expected of expectedBlanks) {
                const submitted = answer.blanks.find((b) => b.index === expected.index);
                if (!submitted ||
                    !submitted.answer ||
...
async function validateCourseLongAnswer(questionText, data, answer) {
    const rubric = data.rubric || [];
    const goldenSolution = data.goldenSolution || "";
    const traineeAnswer = answer?.answer || "";
    ...
    const rubricDescription = rubric
        .map((r, i) => `${i + 1}. "${r.title}" (weight: ${r.weight}%): ${r.description || "No description"}`)
        .join("\n");
...
async function isContentUnlocked(traineeProgress, itemId, type, projectId) {
    ...
    const currentContent = publishedContents.find((c) => c.id === traineeProgress.currentContentId);
    if (!currentContent)
        return false;
    const targetContent = publishedContents.find((c) => {
        if (type === "QUIZ")
            return c.quizId === itemId;
        if (type === "COURSE")
            return c.courseId === itemId;
        return false;
    });
    if (!targetContent)
        return false;
...
async function advanceToNextContent(traineeProgressId, currentContentId, projectId) {
    const contents = await prisma.projectContent.findMany({
        where: { projectId },
        include: {
            quiz: { select: { id: true, published: true } },
    ...
    const currentIndex = publishedContents.findIndex((c) => c.id === currentContentId);
    if (currentIndex === -1)
        return;
        ...
        const sidebarItems = publishedContents.map((content) => {
            ...
            if (progress) {
                if (content.type === "COURSE" && content.courseId) {
                    const cp = progress.courseProgresses.find((p) => p.courseId === content.courseId);
                    if (cp) {
                    ...
                    const qa = progress.quizAttempts.find((a) => a.quizId === content.quizId);
                    if (qa) {
                ...
                if (status === "locked" && progress.currentContentId) {
                    const currentPos = publishedContents.find((c) => c.id === progress.currentContentId)?.position;
...
export const getCourseProgress = async (req, res) => {
    try {
        ...
        const contentUnlocked = await isContentUnlocked(traineeProgress, courseId, "COURSE", projectId);
        if (!contentUnlocked) {
...
export const getCourseSectionQuestions = async (req, res) => {
    try {
        ...
        const completedQuestionIds = new Set(courseProgress.completions.map((c) => c.courseQuestionId));
        const questions = section.questions.map((q) => ({
            id: q.id,
            type: q.type,
            question: q.question,
            order: q.order,
            data: stripAnswers(q.type, q.data),
            completed: completedQuestionIds.has(q.id),
        }));
        const answerableQuestions = section.questions.filter((q) => q.type !== "content_block");
...
export const submitCourseAnswer = async (req, res) => {
    try {
        ...
        if (question.type === "long_answer") {
            result = await validateCourseLongAnswer(question.question, question.data, answer);
        }
        else {
            result = validateCourseAnswer(question.type, question.data, answer);
        }
        if (!result.correct) {
            res.status(200).json({
                correct: false,
        ...
        if (sectionCompleted) {
            ...
            if (isLastSection) {
                ...
                await advanceToNextContent(traineeProgress.id, traineeProgress.currentContentId, projectId);
                res.status(200).json({
                    correct: true,
                    message: result.message,
...
export const getQuizProgress = async (req, res) => {
    try {
        ...
        const contentUnlocked = await isContentUnlocked(traineeProgress, quizId, "QUIZ", projectId);
        if (!contentUnlocked) {
...
export const getQuizSectionQuestions = async (req, res) => {
    try {
        ...
        const responseMap = new Map(quizAttempt.responses.map((r) => [r.questionId, r]));
        const questions = section.questions.map((q) => {
...
export const completeQuiz = async (req, res) => {
    try {
        ...
        if (quizAttempt.status !== "IN_PROGRESS") {
            res.status(400).json({
                message: quizAttempt.status === "SUBMITTED"
        ...
        res.status(200).json({
            message: "Quiz submitted successfully. Your results will be available shortly.",
            status: "SUBMITTED",
        });
    }
    catch (error) {
    ...
    try {
        ...
        res.status(200).json({
            ...
            result: {
                totalScore: quizAttempt.totalScore,
                maxScore: quizAttempt.maxScore,
                scorePercent: quizAttempt.maxScore && quizAttempt.maxScore > 0
                    ? Math.round(((quizAttempt.totalScore || 0) / quizAttempt.maxScore) * 10000) / 100
                    : 0,
                passed: quizAttempt.passed,
...
export const getContentDetails = async (req, res) => {
    try {
        ...
        // Check if unlocked
        const isUnlocked = await isContentUnlocked(traineeProgress, content.type === "QUIZ" ? content.quizId : content.courseId, content.type, projectId);
        if (!isUnlocked) {
        ...
        if (content.type === "COURSE" && content.courseId) {
            ...
            progressData = courseProgress
                ? {
                    ...
                    startedAt: courseProgress.startedAt,
                    completedAt: courseProgress.completedAt,
                    completedQuestionIds: courseProgress.completions.map((c) => c.courseQuestionId),
                }
                : null;
        }
        else if (content.type === "QUIZ" && content.quizId) {
...
export const completeCourseSection = async (req, res) => {
    try {
        const { projectId, courseId, sectionId } = req.params;
        const traineeId = req.userId;
        if (typeof projectId !== "string" ||
