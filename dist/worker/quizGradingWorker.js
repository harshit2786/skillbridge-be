...
async function gradeQuestion(questionType, questionText, questionData, questionPoints, response) {
    switch (questionType) {
        case "mcq": {
            ...
            return {
                ...
                feedback: isCorrect
                    ...
                    : `Incorrect. The correct answer(s): ${correctOptionIds
                        .map((id) => questionData.options.find((o) => o.id === id)?.text)
                        .join(", ")}`,
        ...
        case "fill_ups": {
            const expectedBlanks = questionData.blanks || [];
            const submittedBlanks = response?.blanks || [];
            let allCorrect = true;
            const blankResults = [];
            for (const expected of expectedBlanks) {
                const submitted = submittedBlanks.find((b) => b.index === expected.index);
                if (!submitted ||
                    !submitted.answer ||
                    submitted.answer.trim().toLowerCase() !==
                        expected.answer.trim().toLowerCase()) {
                    allCorrect = false;
                    blankResults.push(`Blank ${expected.index + 1}: Expected "${expected.answer}", got "${submitted?.answer || "(empty)"}"`);
                }
                else {
        ...
        case "long_answer": {
            return await gradeLongAnswer(questionText, questionData, questionPoints, response);
        }
        case "content_block": {
            return {
                pointsAwarded: 0,
...
async function gradeLongAnswer(questionText, questionData, questionPoints, response) {
    const rubric = questionData.rubric || [];
    const goldenSolution = questionData.goldenSolution || "";
    const traineeAnswer = response?.answer || "";
  ...
  "overallFeedback": "1-2 sentence summary feedback for the student"
    ...
    try {
        ...
        for (const criterion of criteria) {
            const rubricItem = rubric.find((r) => r.title.toLowerCase() === criterion.title?.toLowerCase());
            if (rubricItem && criterion.met) {
                totalWeightMet += rubricItem.weight;
            }
            feedbackParts.push(`• ${criterion.title}: ${criterion.met ? "✓ Met" : "✗ Not met"} — ${criterion.reasoning || ""}`);
        }
...
// ─── Worker Process ──────────────────────────────────────────────
const processQuizGrading = async (job) => {
    ...
    try {
        ...
        if (quizAttempt.status !== "SUBMITTED") {
            console.log(`[QuizGrading] Attempt ${quizAttemptId} status is ${quizAttempt.status}, skipping`);
            return;
        }
        let totalScore = 0;
        let maxScore = 0;
        for (const response of quizAttempt.responses) {
            ...
            const gradeResult = await gradeQuestion(question.type, question.question, question.data, question.points, response.response);
        ...
        if (passed) {
            ...
            if (traineeProgress?.currentContentId) {
                await advanceToNextContent(traineeProgressId, traineeProgress.currentContentId, projectId);
            }
        }
        console.log(`✅ [QuizGrading] Completed: ${quizAttemptId} — ${totalScore}/${maxScore} (${scorePercent.toFixed(1)}%) — ${passed ? "PASSED" : "FAILED"}`);
    }
    catch (error) {
        console.error(`❌ [QuizGrading] Error grading attempt ${quizAttemptId}:`, error);
        throw error;
    }
};
// ─── Start Worker ────────────────────────────────────────────────
...
//# sourceMappingURL=quizGradingWorker.js.map